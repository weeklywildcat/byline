...
describe('application commands and mention safety', () => {
  it('generates every required top-level guild command once', () => expect(commandDefinitions().map((command) => command.name)).toEqual(['story', 'stories', 'editing', 'announce', 'sync', 'Create Byline story']));

  it('uses publication identity in the context menu without exceeding Discord limits', () => {
    const command = commandDefinitions('Harbor Light Student Newspaper').at(-1);
    expect(command?.name).toBe('Create Harbor Light Student News');
