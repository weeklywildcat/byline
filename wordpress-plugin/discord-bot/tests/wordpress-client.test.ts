...
describe('WordPress boundary', () => {
  ...
  it('uses a stable request id for idempotent story creation', async () => { const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify({ created: false, story: { id: 9 } }), { status: 200, headers: { 'content-type': 'application/json' } })); await new WordPressClient(config, fetcher).createStory('12345678901234567', 'Pitch', '22345678901234567', 'promote-123'); expect(fetcher.mock.calls[0]![1].headers['x-wwh-request-id']).toBe('promote-123'); });
