...
function byline_distribution_safe_status($value): string
{
    ...
    return in_array($value, ['not_published', 'published', 'rebuild_pending', 'live', 'build_failed', 'needs_configuration', 'ready', 'not_configured', 'pending', 'sent', 'skipped', 'failed'], true)
        ? $value
        : 'not_configured';
}
...
function byline_distribution_channel_descriptors(int $post_id = 0): array
{
    ...
    if (is_object($post) && (string) ($post->post_status ?? '') === 'publish') {
        ...
        $lifecycle = function_exists('byline_deployment_lifecycle_status')
            ...
            : 'unknown';
        // A manifest that proves the expected revision is the strongest
        // evidence there is, so it is checked first: a leftover scheduled event
        // must not keep an already-live story reading as still building.
        if ($lifecycle === 'live') {
            $website['status'] = 'live';
        } elseif ($lifecycle === 'needs_configuration') {
            // Published in WordPress, with a durably recorded revision the site
            // still owes and nowhere to send it. Never "live", never "failed".
            $website['status'] = 'needs_configuration';
        ...
        } elseif ($lifecycle === 'unknown'
            // Older installations have no recorded expected revision at all.
            // Keep their established reachable-manifest behavior, but only
            // there, and only after the stronger signals above: every revision
            // generated by a revision-aware Byline is recorded durably, so this
            // fallback can no longer report a stale manifest as live.
