...
export function MorePackage({ package: resolved }: MorePackageRendererProps) {
  ...
  // A migrated v1 story-grid had no archive link, utility rail, or intermediate
  // layout/list wrapper. Keep that visible shape available from semantic props
  // so the fallback can eventually be retired without changing its markup.
  const legacySimpleGrid = !hasUtility && !resolved.archiveLink.enabled;
