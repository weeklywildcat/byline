...
vi.mock("@/lib/publication", () => ({
  getPublicationConfig: () => ({ urls: { publicSite: "https://weeklywildcat.com" } })
}));
...
const originalWordPressApiUrl = process.env.NEXT_PUBLIC_WP_API_URL;

beforeEach(() => {
  vi.resetModules();
  process.env.BYLINE_WORDPRESS_FETCH_CONCURRENCY = "2";
  process.env.WORDPRESS_FETCH_CACHE_KEY = "test-build";
  process.env.NEXT_PUBLIC_WP_API_URL = "https://cms.example.test/wp-json/wp/v2";
...
afterEach(() => {
  ...
  if (originalWordPressApiUrl === undefined) delete process.env.NEXT_PUBLIC_WP_API_URL;
  else process.env.NEXT_PUBLIC_WP_API_URL = originalWordPressApiUrl;
});

function paginatedFetchTracker() {
...
describe("WordPress build load controls", () => {
  ...
  it("bounds concurrent requests while loading every sports page", async () => {
    const tracker = paginatedFetchTracker();

    const { getAllSportsGames } = await import("@/lib/headless");
    const games = await getAllSportsGames();

    expect(games).toHaveLength(6);
    expect(tracker.requestedPages.sort((left, right) => left - right)).toEqual([1, 2, 3, 4, 5, 6]);
    expect(tracker.fetchMock).toHaveBeenCalledTimes(6);
    expect(tracker.getMaxActiveRequests()).toBeLessThanOrEqual(2);
  });

  it("bounds concurrent requests while loading standard WordPress collections", async () => {
    const tracker = paginatedFetchTracker();
