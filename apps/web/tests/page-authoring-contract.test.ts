...
describe("native WordPress Page authoring contract", () => {
  ...
  it("keeps the Page REST shape and Gutenberg controls aligned", () => {
    ...
    expect(pageMigration).toContain("const BYLINE_WEEKLY_PAGE_SEED_VERSION = 2;");
    expect(pageMigration).toContain("const BYLINE_WEEKLY_PAGE_MIGRATION_VERSION = 3;");
    expect(pageMigration).toContain("function byline_correct_weekly_page_blocks()");
    expect(pageMigration).toContain("parse_blocks($content)");
    expect(pageMigration).toContain("serialize_blocks($transformed['blocks'])");
    expect(pageMigration).toContain("array_fill(0, count($inner_blocks), null)");
    ...
    expect(pageSectionMetadata.render).toBe("file:./render.php");
    expect(pageBlocks).toContain("register_block_style('core/buttons'");
    expect(pageBlocks).not.toContain("register_block_style('byline/page-section'");
  });
});
