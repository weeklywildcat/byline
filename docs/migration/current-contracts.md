...
when enabled, its client calls the static host's relative `/api` integration.
Byline core itself still requires no public server.
...
diagnostics, design, deployment, teams, games, rosters, events, authors, and
Discord equivalents. Legacy routes and REST fields remain aliases for rolling
frontend and bot upgrades.
